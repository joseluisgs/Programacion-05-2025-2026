...
public class StrikeShipService {
    ...
    public void Simulate() {
        ...
        while (LeftEnemies > 0 && time < _timeMax * 1000) {
            WriteLine($"Tiempo: {time}");
            WriteLine($"Enemigos: {LeftEnemies}");

            if (time % 300 == 0) {
                WriteLine("Enemigos se reubican en el espacio...");
                PlaceEnemies();
            }
            ...
            if (_map[row, col] is { } enemy) {
                WriteLine($"Tiene un enemigo en: {row + 1}, {col + 1}");
                _numberOfHits++;
                var effectiveDamage = 0;
                WriteLine("Enemigo: " + enemy);

                if (enemy.Rol is IDefensa droideConDefensa) {
                    WriteLine("El enemigo está intentando defenderse...");
                    effectiveDamage = droideConDefensa.Defend(shotValue);
                }

                if (enemy.Rol is IEscudo droideConEscudo) {
                    WriteLine($"El enemigo esta usando su escudo: {droideConEscudo.Shield}");
                    effectiveDamage = droideConEscudo.UsingShield(shotValue);
                }

                if (enemy.Rol is IMovimiento droideConMovimiento) {
                    WriteLine("El enemigo está intentando escapar...");
                    if (!droideConMovimiento.Move()) {
                        WriteLine("El enemigo no ha podido escapar.");
                        effectiveDamage = shotValue;
                    }
                }

                enemy.MaxEnergy -= effectiveDamage;
                WriteLine($"Daño efectivo: {effectiveDamage}");
                WriteLine("Estado del enemigo tras el ataque: " + enemy);

                if (enemy.Rol is RolSw6969 sw6969) {
                    WriteLine("El enemigo es muy inestable, puede explotar.");
                    if (sw6969.Explode()) {
                        enemy.MaxEnergy = 0;
                        WriteLine("El enemigo ha explotado.");
                    }
                }
                
                // Posibilidad de que el enemigo se repare y cambie de rol
                if (time % 200 == 0 && _random.Next(1, 101) < 50) {
                    WriteLine("🔄¡El enemigo se está reparando y cambia sus características!🔄");
                    WriteLine("Antiguo rol: " + enemy.Rol.GetType().Name);
                    enemy.Rol = DroidFactory.GetRandomRol();
                    WriteLine("Nuevo rol: " + enemy.Rol.GetType().Name);
                }
            }

            else {
                WriteLine("¡Disparo en el vacío!");
            }
    ...
    private int GiveMeAShot() {
        if (new Random().Next(0, 101) <= 15) {
            WriteLine("¡Has conseguido un disparo crítico!");
            return 50;
        }

        WriteLine("Disparo normal.");
        return 25;
    }
        ...
        for (var row = 0; row < _mapSize; row++) {
            for (var col = 0; col < _mapSize; col++) {
                ...
                if (droid == null)
                    ...
                    cell = $"[  {droid.Simbolo}  ]"; // 2 espacios antes/después emoji
                Write(cell);
            }
