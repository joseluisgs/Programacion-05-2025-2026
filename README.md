...
- [Programación - 05 Programación Avanzada Orientada a Objetos](#programación---05-programación-avanzada-orientada-a-objetos)
  ...
  - [14. objects vs dynamic](#14-objects-vs-dynamic)
    - [14.1. Uso de `object`](#141-uso-de-object)
    - [14.2. Uso de `dynamic`](#142-uso-de-dynamic)
  - [16. Mejorando la Configuración de nuestros Proyectos](#16-mejorando-la-configuración-de-nuestros-proyectos)
...
## 14. objects vs dynamic

En C#, `object` es la clase base de todos los tipos. Todos los tipos, ya sean primitivos (int, bool, etc.) o complejos (clases, arrays), derivan de `object`. Esto significa que cualquier variable puede ser tratada como un `object`.

### 14.1. Uso de `object`
El uso de `object` permite almacenar cualquier tipo de dato en una variable, pero requiere casting explícito para recuperar el tipo original. Es útil cuando se necesita almacenar datos heterogéneos en una colección o cuando se trabaja con APIs que manejan tipos genéricos. Debemos tener cuidado con el rendimiento y la seguridad de tipos, ya que el casting incorrecto puede llevar a excepciones en tiempo de ejecución. Es más seguro/rapido que `dynamic`, pero menos flexible.

```csharp
object miObjeto = 42; // Almacena un int como object
int numero = (int)miObjeto; // Necesita casting explícito

object miCadena = "Hola Mundo"; // Almacena un string como object
string texto = (string)miCadena; // Necesita casting explícito

object miLista = new int[] { 1, 2, 3 }; // Almacena un array como object
int[] numeros = (int[])miLista; // Necesita casting explícito
```

### 14.2. Uso de `dynamic`
`dynamic` es un tipo especial que permite la resolución de tipos en tiempo de ejecución. No requiere casting explícito, pero puede llevar a errores en tiempo de ejecución si el tipo no es compatible. Es útil cuando se trabaja con datos que pueden cambiar de tipo o cuando se interactúa con bibliotecas dinámicas o COM, o interoperabilidad con lenguajes dinámicos: javaScript, Python, etc.

Se recomienda no usarlos, salvo casos justificados y puede ocasionar algún problemas de rendimiento.

```csharp
dynamic miDinamico = 42; // Almacena un int como dynamic
int numeroDinamico = miDinamico; // No necesita casting explícito

dynamic miCadenaDinamica = "Hola Mundo"; // Almacena un string como dynamic
string textoDinamico = miCadenaDinamica; // No necesita casting explícito
dynamic miListaDinamica = new int[] { 1, 2, 3 }; // Almacena un array como dynamic
int[] numerosDinamicos = miListaDinamica; // No necesita casting explícito
```

## 16. Mejorando la Configuración de nuestros Proyectos

Para mejorar la configuración de nuestros proyectos en C#, podemos ajustar varias opciones en el archivo de proyecto (.csproj) o mediante la interfaz de Visual Studio. Aquí hay algunas recomendaciones:
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <PropertyGroup>
    ...
    <Nullable>enable</Nullable> <!-- Habilitar referencias anulables -->
    <TreatWarningsAsErrors>true</TreatWarningsAsErrors> <!-- Tratar advertencias como errores -->
    <AnalysisMode>AllEnabledByDefault</AnalysisMode> <!-- Habilitar análisis de código -->
    <ImplicitUsings>enable</ImplicitUsings> <!-- Habilitar importaciones implícitas -->
...
- Ajustar la configuración del proyecto mejora la calidad y el rendimiento del código.
